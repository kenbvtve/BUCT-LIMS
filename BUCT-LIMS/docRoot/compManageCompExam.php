                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                if(isset($_SESSION['login_userType'])&&$_SESSION['login_userType']==1){
                                    ...
                                    $res=$compjoin_tb_serv->getCompjoinTbByCompjoinStatus('审核中');
                                    ...
                                    for ($i=0;$i<count($res);$i++) {
                                        ...
                                        echo "<tr class='gradeX'><td>$j</td><td><a href='compManageCompInfo.php?comp_id={$row['comp_id']}'>".$compName."</a></td><td><a href='compManageCompExamInfoTable.php?compjoin_id={$row['compjoin_id']}'>{$row['compjoin_id']}</a></td><td>{$row['compjoin_leaderName']}</td><td>{$row['compjoin_leaderId']}</td><td>{$row['compjoin_teaName']}</td><td><a onclick=\"return confirm('该操作不可逆，确认拒绝？');\" href='../control/compManageCompExamProcess.php?flag=1&compjoin_id={$row['compjoin_id']}' class='btn btn-danger btn-xs'>拒绝</a>&nbsp;<a href='../control/compManageCompExamProcess.php?flag=2&compjoin_id={$row['compjoin_id']}' class='btn btn-info btn-xs'>同意</a></td></tr>";
                                    }
                                }
                                else{
