...
if (isset($_SESSION['userManageUserStatusYesProcess_err'])){
    switch ($_SESSION['userManageUserStatusYesProcess_err']){
        case 1:case 3:
    ...
    unset($_SESSION['userManageUserStatusYesProcess_err']);
}
                ...
                <div class="panel panel-default">
                    <div class="panel-body">
                        <ul class="nav">
                            ...
                            <li><a href="userManageUserStatusYes.php" class="menu-top-active">审核通过用户</a></li>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                if (isset($_SESSION['login_userType'])&&$_SESSION['login_userType']==1){
                                    ...
                                    $res=$user_tb_serv->getUserListWaitExamByUserStatus(1);
                                    ...
                                    for ($i=0;$i<count($res);$i++) {
                                        ...
                                        echo "<tr class='gradeX'><td>$j</td><td>{$row['user_id']}</td><td>$userType</td><td>{$row['user_name']}</td><td>{$row['user_college']}</td></td><td><a onclick=\"return confirm('确认删除？');\" href='../control/userManageUserStatusYesProcess.php?flag=1&user_id={$row['user_id']}' class='btn btn-danger btn-xs'>删除</a>&nbsp;<a  onclick=\"return confirm('确认禁用？');\" href='../control/userManageUserStatusYesProcess.php?flag=2&user_id={$row['user_id']}' class='btn btn-primary btn-xs'>禁用</a></td></tr>";
                                    }
                                }
                                else{
